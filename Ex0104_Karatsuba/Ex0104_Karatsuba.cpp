...
string KaratsubaHelper(string str1, string str2, int level) // level은 디버깅용
{
	...
	string midterm =   Subtract(KaratsubaHelper(Add(a,b), Add(c,d),level+1),Add(ac,bd));
	// 문자열 뒤에 '0'을 추가해서 10^N를 O(N)으로 처리
	// TODO: ..
	ac.append(string((N - mid) * 2, '0'));
	midterm.append(string((N-mid),'0'));
	// string result = TODO;
	string result = Add(Add(ac,midterm),bd);
