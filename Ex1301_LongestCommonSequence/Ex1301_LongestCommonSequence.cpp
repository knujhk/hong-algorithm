...
int RecurLengthLCS(const string& X, const string& Y, int m, int n)
{
	...
	if (X[m - 1] == Y[n - 1])
		return RecurLengthLCS(X.substr(0,X.size()-1),Y.substr(0,Y.size()-1),m-1,n-1) + 1;
	else
		return max(RecurLengthLCS(X.substr(0,X.size()-1),Y,m-1,n), RecurLengthLCS(X,Y.substr(0,Y.size()-1),m,n-1));
...
int BottomUpLCS(const string& X, const string& Y, int m, int n)
{
	...
	for (int i = 1; i <= m; i++)
	{
		for (int j = 1; j <= n; j++)
		{
			...
			if (X[i - 1] == Y[j - 1]) // 주의: i - 1, j - 1, 1을 빼야 string에서의 인덱스임
			{
