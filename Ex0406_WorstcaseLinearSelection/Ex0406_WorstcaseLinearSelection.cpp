...
Pair Select(vector<int>& arr, int lo, int hi, int k)
{
	...
	while ((hi - lo + 1) % 5 != 0)
	{
		SelectionSortPass(arr, lo, hi);	//가장 작은 것 하나 골라서 lo로 보내기
	...
	//각 그룹의 첫번째 요소는 lo~(lo+g-1)에 연속적으로 존재
	for(int i = 0; i < g; i++){
		for(int j = 0; j < 4; j++){	//한 그룹 안에서 정렬
			SelectionSortPass(arr,lo+i+j*g,(hi-g)+1+i,g);
		}
	}
	...
	// Pair x = TODO
	Pair x = Select(arr,lo+g*2,lo+g*3-1,int(std::ceil(g/2.0)));	//한 그룹은 전체 n에서 축소된 것이기 때문에 전체중의 k번째를 찾고싶다고해서
												//여기 한 그룹에서 k번째를 찾으면 오류다 
												//k번째 값의 전체 n에 대한 상대적 위치(비율)을 적용해야한다
...
int main()
{
	...
	for (int n = 10000000; n <= 10000000; n += 1)
	{
