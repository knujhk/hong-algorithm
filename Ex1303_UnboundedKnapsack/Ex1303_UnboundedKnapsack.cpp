...
int RecurUnboundedKnapsack(vector<int> weights, vector<int> values, int W, int n)
{
	...
	{
		// int max = RecurUnboundedKnapsack(weights, values, W, n - 1);
		// int k = 1;
		// while(k * weights[n-1] <= W){
		// 	int pos = RecurUnboundedKnapsack(weights, values, W - k * weights[n - 1], n - 1) + k * values[n - 1];
		// 	if(pos > max)
		// 		max = pos;
		// 	k++;
		// }
		...
		return max(RecurUnboundedKnapsack(weights, values, W, n - 1),RecurUnboundedKnapsack(weights, values, W-weights[n-1], n)+values[n-1]);
	}
}
	...
	for (int w = 1; w <= W; w++)
	{
		for (int n = 1; n <= weights.size(); n++)
		{
			...
			else
			{
				...
				// while(k * weights[n-1] <= w){
				// 	int pos = table[w - k * weights[n - 1]][n - 1] + k* values[n - 1];
				// 	if(pos > max)
				// 		max = pos;
				// 	k++;
				// }
				// k--;
				// table[w][n] = max;
				// items[w][n] = items[w-k*weights[n-1]][n-1];
				// items[w][n][n-1] = k;
